...
def render_letterhead_png(v, out_path):
    ...
    Letterhead SVG viewBox is "210 210 280 171"; this canvas mirrors it.
    LK uses Courier New at fs=240 (vb), letter-spacing −0.02em, baseline y=350.
    Tagline uses Inter Thin at fs=28 (vb), baseline y=377, nudged right a touch.
    """
    # Canvas sized to match the letterhead viewBox 280×171.
    W = 3000
    F = W / 280.0  # px per viewBox unit
    H = int(round(171 * F))  # ≈ 1832
    ...
    #   base draw x = 214.95 − 3.17 = 211.78, nudged right +3 → 214.78
...
SIG_LOGO_H = 103                 # rendered image display height (gold SVG viewBox 160:103)
...
def render_signature_image(out_path, layout="A"):
    ...
    # the content. Aspect ratio 160:103 comes from the gold SVG viewBox.
    LOGO_H = TEXT_H
    LOGO_W = int(round(LOGO_H * 160 / 103))
